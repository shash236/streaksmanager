...
import { toEpoch, fromEpoch, isSameDay } from '../utils/dateUtils';
...
const CalendarModal: React.FC<CalendarModalProps> = ({ isOpen, onClose, streakId, streakTitle, currentStreak, longestStreak, onUpdate, readOnly }) => {
    ...
    const fetchHistory = async () => {
        ...
        try {
            ...
            const entries = await streakService.getHistory(
                streakId,
                undefined,
                toEpoch(formatDate(firstDay)),
                toEpoch(formatDate(lastDay))
            );

            const historySet = new Set(entries.map(e => fromEpoch(e.checkInDate)));
    ...
    const handleCheckToggle = async (day: number) => {
        ...
        try {
            ...
            if (isChecked) {
                ...
                await streakService.uncheck(streakId, toEpoch(dateStr));
            } else {
                newHistory.add(dateStr);
                await streakService.checkIn(streakId, toEpoch(dateStr));
            }
    ...
    const isToday = (day: number) => {
        const today = new Date();
        return isSameDay(today, new Date(currentDate.getFullYear(), currentDate.getMonth(), day));
    };
